...
#'data(loans)
#'#first select the model than scale the probabilities
...
#'auc.model(predictions = loans$probs, observed = res$dev.db$Creditability)
#'auc.model(predictions = loans$score, observed = ifelse(res$dev.db$Creditability == 0, 1, 0))
#'#note that score higher score indicates lower probability of default
#'@export
